...
class Pitch(object):
    ...
    def lines(self, xstart, ystart, xend, yend, color=None, n_segments=100,
        ...
        **kwargs : All other keyword arguments are passed on to matplotlib.collections.LineCollection.
        ...
        if (opp_comet or transparent) and (lw.size > 1):
            raise NotImplementedError("Multiple linewidths with an opposite comet or transparent line is not implemented.")
        ...
        if (opp_comet or transparent) and (cmap is None) and (to_rgba_array(color).shape[0] > 1):
            raise NotImplementedError("Multiple colors with an opposite comet or transparent line is not implemented.") 
        ...
        # create segments 2
        if (transparent is False) and (opp_comet is False) and (cmap is None):
            if self.orientation == 'horizontal':
                segments = np.transpose(np.array([[xstart, ystart], [xend, yend]]), (2, 0, 1))
            elif self.orientation == 'vertical':
                segments = np.transpose(np.array([[ystart, xstart], [yend, xend]]), (2, 0, 1))
        ...
        if opp_transparent:
            ...
            if cmap is None:
                color = to_rgb(color)
                cmap = self._create_transparent_cmap(color, n_segments, alpha_end, alpha_start)
            else:
                if isinstance(cmap, str):
                    cmap = get_cmap(cmap)
                elif not isinstance(cmap, (ListedColormap, LinearSegmentedColormap)):
                ...
                cmap = cmap(np.linspace(1, 0, n_segments))
